...
@Component({
  template: `
    ...
    <div *ngIf="formControl.value">
      ...
      <br />
      <button class="btn btn-primary" (click)="saveTierPrices()">
        Save
      </button>
      <br /><br />
    </div>
    ...
    this.route.paramMap.subscribe((paramMap) => {
      ...
      if (this.productId) {
        this.dataService
          ...
          >(
            parse(
              `query TierPrices($productId: ID!){
                productTierPrices(productId: $productId){
                  productVariant{
                    sku
                  }
                  quantity
          ...
          .single$.toPromise()
          .then((response) => {
            this.discounts = response.productTierPrices
          ...
          .catch((e) => {
            notify({
              message: "Tier prices couldn't be fetched. Check the console.",
              type: "error",
            });
            console.error(e);
          });
  ...
  saveTierPrices() {
    if (this.productId) {
      this.dataService
        .mutate<
          {
            updateProductTierPrice: boolean;
          },
          {
        ...
        >(
          parse(
            `mutation updateProductVariantTierPricesBySku($productVariantSku: String!, $discounts: [TierPriceInput!]!) {
              updateProductVariantTierPricesBySku(productVariantSku: $productVariantSku, discounts: $discounts)
            }`
          ),
          {
...
export class TierPriceInputModule {}
