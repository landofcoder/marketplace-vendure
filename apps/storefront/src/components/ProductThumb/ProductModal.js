...
import { formatterConvertCurrency, getProductCartQuantity } from "../../lib/product";
import { ProductRating } from "@bavaan/storefront-base/src/components/Product";
import RenderOptionSelect from "../ProductDetails/RenderOptionSelect";
...
const ProductModal = (props) => {
  ...
  if (productQuickView && productQuickView.variants.length > 0) {
    formatter = formatterConvertCurrency(productQuickView.variants[0].currencyCode)
  }
  useEffect(() => {
    if (productQuickView && productQuickView.variants && selectedProduct == null) {
      console.log("SET STATE")
      setSelectedProduct(productQuickView.variants[0]);
    }
  }, [productQuickView])
  return (
    <Modal
          ...
          <Col md={7} sm={12} className="ml-auto">
            <CustomScroll allowOuterScroll={true}>
              <div className="product-quickview__content">
                ...
                <div className="product-quickview__price space-mb--20">
                  {product.discount > 0 ? (
                    <Fragment>
                      ...
                      <span className="main-price">{formatter.format(selectedProduct?.priceWithTax)}</span>
                    </Fragment>
                  ) : (
                ...
                {!_.isEmpty(productQuickView) && !_.isEmpty(selectedProduct) ? (
                  <RenderOptionSelect
                  selectedProduct={selectedProduct}
                  setSelectedProduct={setSelectedProduct}
                  productVariantList={productQuickView.variants}
                />
                ): null}
                {product.variation ? (
